...
if sub_email:
    if approve == "Approve":
        st.info("You selected to approve the loan. (Email generation for approval not implemented.)")
    elif approve == "Reject":
        df_sub = st.session_state.get("df_sub")
        if df_sub is None:
            st.warning("No prediction data available. Please submit the application form first.")
            st.stop()
